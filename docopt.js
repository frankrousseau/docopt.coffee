...
var AnyOptions, Argument, Command, Dict, DocoptExit, DocoptLanguageError, Either, OneOrMore, Option, Optional, Pattern, Required, TokenStream, atos, docopt, extras, formal_usage, parse_args, parse_atom, parse_doc_options, parse_expr, parse_long, parse_pattern, parse_seq, parse_shorts, print, printable_usage, _ref, _ref1, _ref2, _ref3, _ref4,
...
Pattern = (function() {
  ...
  Pattern.prototype.fix_identities = function(uniq) {
    var child, enumerate, flat, i, pattern, _i, _j, _len, _len1, _ref, _ref1;
    if (uniq == null) {
      uniq = null;
    }
    if (!this.hasOwnProperty('children')) {
    ...
    for (_j = 0, _len1 = enumerate.length; _j < _len1; _j++) {
      _ref1 = enumerate[_j], i = _ref1[0], child = _ref1[1];
      if (!child.hasOwnProperty('children')) {
        this.children[i] = uniq[child];
      } else {
  ...
  Pattern.prototype.either = function() {
    var child, children, e, either, group, groups, i, indices, name, oneormore, optional, required, ret, types, zip, _i, _j, _len, _len1, _ref, _ref1, _ref2;
    if (!this.hasOwnProperty('children')) {
      ...
      while (groups.length) {
        ...
        for (_i = 0, _len = zip.length; _i < _len; _i++) {
          _ref1 = zip[_i], i = _ref1[0], child = _ref1[1];
...
Command = (function(_super) {
  ...
  Command.prototype.match = function(left, collected) {
    ...
    if (!args.length || (args[0].value !== this.name())) {
      for (_i = 0, _len = collected.length; _i < _len; _i++) {
        cmd = collected[_i];
        if ((cmd != null ? cmd.cmdname : void 0) === this.name()) {
          return [true, left, collected];
        }
      }
      return [false, left, collected];
...
AnyOptions = (function(_super) {
  ...
  function AnyOptions() {
    _ref = AnyOptions.__super__.constructor.apply(this, arguments);
    return _ref;
  }
...
Required = (function(_super) {
  ...
  function Required() {
    _ref1 = Required.__super__.constructor.apply(this, arguments);
    return _ref1;
  }

  Required.prototype.match = function(left, collected) {
    var c, child, l, matched, _i, _len, _ref2, _ref3;
    if (collected == null) {
      collected = [];
    }
    l = left;
    ...
    for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
      child = _ref2[_i];
      _ref3 = child.match(l, c), matched = _ref3[0], l = _ref3[1], c = _ref3[2];
      if (!matched) {
        return [false, left, collected];
      }
    }
...
Optional = (function(_super) {
  ...
  function Optional() {
    _ref2 = Optional.__super__.constructor.apply(this, arguments);
    return _ref2;
  }

  Optional.prototype.match = function(left, collected) {
    var m, p, _i, _len, _ref3, _ref4;
    if (collected == null) {
      collected = [];
    }
    _ref3 = this.children;
    for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
      p = _ref3[_i];
      _ref4 = p.match(left, collected), m = _ref4[0], left = _ref4[1], collected = _ref4[2];
    }
    return [true, left, collected];
  };
...
OneOrMore = (function(_super) {
  ...
  function OneOrMore() {
    _ref3 = OneOrMore.__super__.constructor.apply(this, arguments);
    return _ref3;
  }

  OneOrMore.prototype.match = function(left, collected) {
    var c, l, l_, matched, times, _ref4;
    if (collected == null) {
      collected = [];
    }
    l = left;
    ...
    times = 0;
    while (matched) {
      _ref4 = this.children[0].match(l, c), matched = _ref4[0], l = _ref4[1], c = _ref4[2];
      times += matched ? 1 : 0;
      if (l_.join(', ') === l.join(', ')) {
        break;
      }
...
Either = (function(_super) {
  ...
  function Either() {
    _ref4 = Either.__super__.constructor.apply(this, arguments);
    return _ref4;
  }

  Either.prototype.match = function(left, collected) {
    var child, outcome, outcomes, _i, _len, _ref5;
    if (collected == null) {
      collected = [];
    }
    outcomes = [];
...
parse_shorts = function(tokens, options) {
  var o, opt, parsed, raw, value, _ref5;
  ...
  while (raw.length > 0) {
    ...
    if (opt.argcount === 0) {
      ...
      _ref5 = [raw, ''], value = _ref5[0], raw = _ref5[1];
    }
    opt.value = value;
    parsed.push(opt);
  }
...
parse_long = function(tokens, options) {
  var o, opt, raw, value, _, _ref5, _ref6;
  _ref6 = (_ref5 = tokens.current().match(/(.*?)=(.*)/)) != null ? _ref5 : [null, tokens.current(), ''], _ = _ref6[0], raw = _ref6[1], value = _ref6[2];
  tokens.shift();
...
parse_seq = function(tokens, options) {
  var atom, result, _ref5;
  result = [];
  while ((_ref5 = tokens.current()) !== null && _ref5 !== ']' && _ref5 !== ')' && _ref5 !== '|') {
...
Dict = (function(_super) {
  ...
  function Dict(pairs) {
    var key, value, _i, _len, _ref5;
    for (_i = 0, _len = pairs.length; _i < _len; _i++) {
      _ref5 = pairs[_i], key = _ref5[0], value = _ref5[1];
      this[key] = value;
    }
  }
...
docopt = function(doc, kwargs) {
  var a, allowedargs, arg, argums, argv, formal_pattern, help, left, matched, name, opt, options, parameters, pot_arguments, pot_options, usage, version, _ref5;
  if (kwargs == null) {
    kwargs = {};
  }
  ...
  argv = kwargs.argv != null ? kwargs.arg : process.argv.slice(2);
  name = kwargs.name != null ? kwargs.name : null;
  help = kwargs.help != null ? kwargs.help : true;
  version = kwargs.version != null ? kwargs.version : null;
  ...
  _ref5 = formal_pattern.fix().match(argv), matched = _ref5[0], left = _ref5[1], argums = _ref5[2];
  if (matched && left.length === 0) {
    ...
    pot_arguments = (function() {
      ...
      _ref6 = formal_pattern.flat();
      _results = [];
      for (_i = 0, _len = _ref6.length; _i < _len; _i++) {
        a = _ref6[_i];
        if ((_ref7 = a.constructor) === Argument || _ref7 === Command) {
          _results.push(a);
        }
      }
      return _results;
